...
object WikipediaRanking {
  ...
  def contains(article: WikipediaArticle, lang: String): Boolean = article.text.split(" +") contains lang
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = 
    rdd.aggregate[Int](0)(
      (acc, article) => if (contains(article, lang)) acc + 1 else acc,
      _ + _
    )
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = 
    langs.map(lang => (lang, occurrencesOfLang(lang, rdd))).sortBy(kv => -kv._2)
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = 
    rdd.flatMap { article =>
      langs.filter(contains(article, _))
      .map((_, article))
    }.groupByKey
